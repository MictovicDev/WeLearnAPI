...
def send_booking_notification_email(self, booking_id, status=None):
    ...
    if not user_email:
        ...
        return
    if status:
        subject = f'Your Booking with {booking.tutor_profile}'
        message = (
                f'You booking has been {status}.\n\n'
                f'Student: {booking.student.get_full_name() or booking.student.username}\n'
                f'Session type: {booking.session_type}\n'
                f'Response note: {booking.tutor_response_note}\n\n'
